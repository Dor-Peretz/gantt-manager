...
export default function App() {
  const { store, viewerEmail, viewer } = useGanttStore();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setHealth(h);
        if (!h.ok) {
          setHint(h.error || "Jira not connected — check JIRA_BASE_URL and credentials in .env");
          return;
        }
        ...
        {!planMode ? (
          <>
            ...
            <button
              ...
              title="Copy a local Gantt Manager link with this JQL pre-filled"
            >
              Share
            </button>
        <button
